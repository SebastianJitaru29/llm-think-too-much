...
def load_df(train: bool = True) -> pd.DataFrame:
    ...
    path =  Path(__file__).parent.parent / "dataset_splitting"

    df = pd.read_parquet(path / f"train.parquet", columns=['question_id', 'target_think_tokens', 'generated_think_tokens', 'is_correct'])
    ...
    return df#pd.concat((df_math, df_aime), ignore_index=True)


def create_regressor_dataset(h: np.ndarray, df: pd.DataFrame) -> tuple[np.ndarray, np.ndarray, tuple[int, ...]]:
    ...
    mask = np.isin(correct.index, h['ids'])
    correct = correct[mask]
    correct = correct.reindex(h["ids"])

    return np.squeeze(h['embeddings']), correct.to_numpy().astype(np.int32), tuple(bins)
...
def load_hidden_states() -> tuple[np.ndarray, np.ndarray]:
    ...
    h_train = np.load("/scratch/s3799042/results_nlp/train_hidden_states.npy", allow_pickle=True).item()#np.load(Path(__file__).parent / "innit_hidden_states" / "hidden_states_math.npy")
...
def get_dataset(train: bool = True) -> tuple[np.ndarray, np.ndarray, tuple[int, ...]]:
    df = load_df(train)
    h = load_hidden_states()

    return create_regressor_dataset(h, df)
